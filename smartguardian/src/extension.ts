...
const AUTH0_DOMAIN =
  process.env.AUTH0_DOMAIN || "dev-az3di7fabdoc8vlz.uk.auth0.com";
const CLIENT_ID = process.env.CLIENT_ID || "BBVCZqG7W4JzbFlhpZDNeRVwV4W2PdPq";
const CLIENT_SECRET =
  process.env.CLIENT_SECRET ||
  "5XtekSVFh634T-e65llkg7RcZW4jQwaPsFvC9pMo053wKB2nVtfs2PusPp3B6lKS";
const CALLBACK_URI =
  process.env.CALLBACK_URI ||
  "vscode://smart-guardian.safeContracts/callback";
...
interface AnalyzeResponse {
  ...
  request_id: string;
  vulnerabilities: VulnerableSnippet[];
}
...
export async function callApiWithSelectedText(
  ...
  const ws = new WebSocket(`ws://localhost:8080/code-detective-model/ws/${requestId}`, {
    headers: {
  ...
  ws.on('message', (message: AnalyzeResponse) => {
...
function handleVulnerabilityData(data: AnalyzeResponse) {
  let vulnerableSnippets: VulnerableSnippet[] = data.vulnerabilities;
